...
st.set_page_config(page_title="MIP PVL Assistant v11.1", layout="wide")

st.title("MIP PVL Assistant v11.1")
st.caption(
    "Protótipo para apoio ao envio de PVL com lógica por tipo de operação, "
    "data de referência, canal e orientação campo a campo do SADIPEM."
)
...
with st.sidebar:
    ...
    st.write(
        "A aba SADIPEM agora cruza tipo de operação e data de referência "
        "para sugerir o que fazer em cada campo crítico."
    )
...
with aba1:
    ...
    m3.metric(
        "Exige garantia da União",
        "Sim"
        if regra.get("garantia_uniao") is True
        else ("Não" if regra.get("garantia_uniao") is False else "Depende"),
    )
...
with aba2:
    ...
    st.write(
        "Envie o MIP em PDF, TXT ou MD. O sistema extrai texto, compara com a base atual, "
        "detecta sinais de mudança e propõe ajustes estruturados por operação."
    )
    ...
    texto_manual = st.text_area(
        ...
        height=180,
        placeholder=(
            "Cole aqui o texto integral ou parcial do MIP "
            "se quiser priorizar o conteúdo colado."
        ),
    )
    ...
    if st.button("Analisar MIP e gerar proposta de atualização"):
        ...
        if not mip_text.strip():
            st.error(
                "Não foi possível obter texto para análise. "
                "Faça upload de um arquivo legível ou cole o texto do MIP."
            )
        else:
            ...
            review_df = build_review_dataframe(
                comparison, section_titles, change_signals
            )
            updates_df = build_structured_update_suggestions(
                mip_text, operacoes_rules
            )
            ...
            if not updates_df.empty:
                ...
                st.info(
                    "Nenhuma sugestão estruturada foi gerada automaticamente "
                    "para o texto analisado."
                )
            ...
            csv_updates = (
                updates_df.to_csv(index=False).encode("utf-8-sig")
                if not updates_df.empty
                else b""
            )
...
with aba3:
    ...
    st.write(
        "Escolha o tipo de operação e a data de referência. "
        "A tela abaixo cruza regras do MIP para dizer o que fazer "
        "em cada campo crítico do SADIPEM."
    )
    ...
    with s2:
        data_ref_sadipem = st.date_input(
            "Data de referência",
            value=date.today(),
            key="sadipem_date",
            format="DD/MM/YYYY",
    ...
    referencias = pd.DataFrame(
        get_reference_period_rules(op_sadipem, data_ref_sadipem)
    )
    plano = pd.DataFrame(
        build_sadipem_action_plan(op_sadipem, data_ref_sadipem, sadipem_df)
    )
    ...
    if not referencias.empty:
        ...
        st.warning(
            "Ainda não há regra temporal detalhada para esta operação. "
            "Precisamos modelar esse rito específico."
        )
    ...
    if not plano.empty:
        ...
        st.download_button(
            ...
            data=csv_plano,
            file_name=(
                f"sadipem_plano_{op_sadipem}_"
                f"{data_ref_sadipem.strftime('%Y%m%d')}.csv"
            ),
            mime="text/csv",
        )
    else:
